...
df = employee_df[ (employee_df["Banner"]=="Wellcome") & (employee_df["Physical Location"]=="Hong Kong") & (employee_df["Budget Group"]=="Store")]
df = df.sort_values(by=["Region", "Area", "Store"])
df = df[df["Area"]!="Multi-area"]
df['Address'] = df.loc[:, 'Region'] + '-' + df.loc[:, 'Area'] + '-' + df.loc[:, 'Store']
...
df['FTE'] = df['Employee Type'].map({"Full-time":1, "Part-time":0.5})
full_time_equivalent = df.groupby('Address')[['FTE']].sum().reset_index().drop(columns=['Address'])
temp = pd.concat([manager_metrics, full_time_equivalent], axis=1, join="inner")

df_final = df_final.merge(temp, left_on='Store', right_on='Address', how='left')
#df_final = df_final.merge(full_time_equivalent, left_on='Store', right_on='Address', how='left')

# Drop the Address column from the merge
df_final.drop(columns=['Address'], inplace=True)


df_final.to_csv(r".\train_wellcome.csv", index=False)
